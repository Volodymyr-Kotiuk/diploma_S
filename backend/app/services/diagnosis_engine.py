...
def _diagnose(node: models.Node | None, metrics: list[models.ResourceMetric], db: Session) -> dict:
    if not metrics:
        return _result("unknown_degradation", "Недостатньо даних", 10, 20, "Для вузла ще немає resource metrics.", [])
    ...
    if (latest.temperature_celsius or 0) > 80:
        ...
        return _result("thermal_risk", "Thermal risk", clamp((latest.temperature_celsius or 80) + 5), 92, "Виявлено ризик перегріву або thermal throttling.", evidence, anomaly)
    ...
    if (latest.ram_usage_percent or 0) > 85 and (latest.swap_usage_percent or 0) > 20:
        ...
        return _result("swap_pressure", "Swap pressure", max(latest.ram_usage_percent or 0, 78), 90, "Вузол компенсує нестачу RAM через swap, що збільшує затримки.", evidence, anomaly)
    ...
    if (latest.ram_usage_percent or 0) > 90 and (latest.ram_available_mb or 0) < max((latest.ram_total_mb or 4096) * 0.12, 512):
        ...
        return _result("memory_pressure", "Memory pressure", latest.ram_usage_percent or 85, 91, "Оперативної пам'яті недостатньо для поточного workload.", evidence, anomaly)

    if (latest.cpu_usage_percent or 0) > 85:
        evidence.extend(
            [
                _ev("cpu_usage_percent", latest.cpu_usage_percent, "> 85", "CPU використовується майже повністю."),
                _ev("load_average_1m", latest.load_average_1m, "informational", "Load average використовується як додатковий доказ, якщо доступний."),
            ]
        )
        return _result("cpu_saturation", "CPU saturation", latest.cpu_usage_percent or 85, 92, "Процесорні ресурси є першопричиною деградації.", evidence, anomaly)
    ...
    if disk_rate > DISK_RATE_HIGH and (latest.cpu_usage_percent or 0) < 80:
        evidence.extend(
            [
                ...
                _ev("cpu_usage_percent", latest.cpu_usage_percent, "< 80", "CPU не є головним bottleneck."),
            ]
        )
        return _result("disk_io_bottleneck", "Disk I/O bottleneck", 76, 86, "Обмеження продуктивності спричинене дисковими операціями.", evidence, anomaly)
    ...
    if net_rate > NETWORK_RATE_HIGH and (latest.cpu_usage_percent or 0) < 80 and (latest.ram_usage_percent or 0) < 85:
        ...
        return _result("network_pressure", "Network pressure", 72, 84, "Основне навантаження припадає на мережевий throughput.", evidence, anomaly)
    ...
    if _avg(sustained_window, "cpu_usage_percent") < 15 and _avg(sustained_window, "ram_usage_percent") < 30:
        evidence.extend(
            [
                ...
                _ev("ram_usage_percent_avg", round(avg_ram, 2), "< 30", "RAM allocation використовується частково."),
            ]
        )
        return _result("underutilization", "Underutilization", 28, 85, "Вузол має надлишково виділені ресурси.", evidence, anomaly)
    ...
    if (latest.service_latency_ms or 0) > 1000 or (latest.service_error_rate or 0) > 5:
        ...
        return _result("service_degradation", "Service degradation", 68, 70, "Сервісні показники деградують, але ресурсна першопричина не підтверджена.", evidence, anomaly)
    ...
    if anomaly.get("is_anomaly") and anomaly.get("anomaly_score", 0) > 60:
        ...
        return _result("unknown_degradation", "Unknown degradation", anomaly["anomaly_score"], 58, "Метрики виглядають нетипово, але конкретний bottleneck не підтверджено.", evidence, anomaly)
    ...
    return _result("healthy", "No critical degradation", 12, 78, "Критичних ознак деградації ресурсів не виявлено.", evidence, anomaly)
...
def _overcommit_evidence(node: models.Node | None, db: Session) -> list[dict]:
    ...
    if node.allocated_cpu_cores and node.max_cpu_cores and node.allocated_cpu_cores > node.max_cpu_cores:
        evidence.append(_ev("allocated_cpu_cores", node.allocated_cpu_cores, f"<= {node.max_cpu_cores}", "CPU allocation перевищує ліміт вузла."))
    if node.allocated_ram_mb and node.max_ram_mb and node.allocated_ram_mb > node.max_ram_mb:
        evidence.append(_ev("allocated_ram_mb", node.allocated_ram_mb, f"<= {node.max_ram_mb}", "RAM allocation перевищує ліміт вузла."))
    if node.environment_id:
        ...
        if total_cpu_limit and total_cpu > total_cpu_limit:
            evidence.append(_ev("environment_allocated_cpu", total_cpu, f"<= {total_cpu_limit}", "Сумарний CPU allocation середовища перевищує доступний CPU limit."))
        if total_ram_limit and total_ram > total_ram_limit:
            evidence.append(_ev("environment_allocated_ram_mb", total_ram, f"<= {total_ram_limit}", "Сумарний RAM allocation середовища перевищує доступний RAM limit."))
    return evidence
